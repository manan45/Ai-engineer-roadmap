...
type Props = { track: Track; query: string; statusMap?: Record<string, LinkStatus> };

export function TrackSection({ track, query, statusMap }: Props) {
  ...
  return (
    <section id={track.id} className="scroll-mt-24">
      <div className="mb-4">
        <h2 className="text-xl font-semibold text-ink">{track.title}</h2>
        {track.blurb && <p className="mt-1 text-sm text-muted max-w-3xl">{track.blurb}</p>}
      </div>
      <div className="grid gap-3">
        {filtered.map((t) => (
          <TopicCard key={t.id} topic={t} statusMap={statusMap} />
        ))}
      </div>
    </section>
  );
